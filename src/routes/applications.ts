...
let APPS = JSON.parse(fs.readFileSync(path.join(__dirname, '../apps/all.json')) as unknown as string) as IApplication[]
...
applicationsRouter.get("/", async (req: Request, res: Response) => {
    try {
        let { id, version } = req.query //pull query strings from request
        let results = APPS
        if (id) {
